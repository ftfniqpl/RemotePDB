...
class Rpdb(pdb.Pdb):
    ...
    def __init__(self, addr="127.0.0.1", remoteaddr=None, port=4444):
    ...
    def toremote(self, remoteaddr, port=4444):
        clientsock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        clientsock.connect((remoteaddr, port))
        return clientsock

    def tolocal(self, listenaddr='127.0.0.1', port=4444):
        self.skt = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        self.skt.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, True)
        self.skt.bind((listenaddr, port))
        self.skt.listen(1)
